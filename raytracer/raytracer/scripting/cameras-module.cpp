...
namespace
{
	...
	{
		Camera perspective(const Point3D& eye, const Point3D& look_at, const Vector3D& up, double distance, double aspect_ratio) const
		{
		...
		OPTIONAL_ARGUMENT(Vector3D, up, Vector3D(0.0, 1.0, 0.0));
		OPTIONAL_ARGUMENT(double, distance, 1);
		OPTIONAL_ARGUMENT(double, aspect_ratio, 1);
		OPTIONAL_ARGUMENT(double, eye_size, 1);
		OPTIONAL_ARGUMENT(Sampler, sampler, samplers::single());
