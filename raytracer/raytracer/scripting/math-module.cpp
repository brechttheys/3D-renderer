...
namespace
{
	...
	struct EasingLibrary {
		...
		EasingFunction cubic_inout() const {
			return easing::cubic_inout();
		}

		EasingFunction bounce(int bounces, double absorption) const {
			return easing::bounce(bounces, absorption);
		}

		EasingFunction elastic(int numberOscillations, double energyLoss) const {
			return easing::elastic(numberOscillations, energyLoss);
		}

		EasingFunction quadratic_in() const {
			return easing::quadratic_in();
		}

		EasingFunction quadratic_out() const {
			return easing::quadratic_out();
		}		
		
		EasingFunction quadratic_inout() const {
			return easing::quadratic_inout();
		}
	...
	void add_easing(Module& module) {
		...
		auto easingLib = std::make_shared<EasingLibrary>();

		module.add_global_const(chaiscript::const_var(easingLib), "Easing");
